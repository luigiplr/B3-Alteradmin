...
# -*- coding: utf-8 -*-
#
# IWNET B3 Plugin Written For AlterAdmin
#
...
class AlterAdminPlugin(b3.plugin.Plugin):
    ...
    _BanList = []
    _parseUserCmdRE = \
        re.compile(r"^(?P<cid>'[^']{2,}'|[0-9]+|[^\s]{2,}|@[0-9]+)(\s+(?P<parms>.*))?$"
                   )
    _currentVote = None
    _caller = None
    _in_progress = False
    ...
    def startup(self):
        ...
        if 'commands' in self.config.sections():
            for cmd in self.config.options('commands'):
                ...
                if func:
                    self._adminPlugin.registerCommand(self, cmd, level,
                            func, alias)

        msg = 'AlterAdmin Admin Plugin v. %s by %s started.' \
    ...
    def onEvent(self, event):
        ...
        if event.type == b3.events.EVT_CLIENT_AUTH:
            ...
            if (event.client.ip or event.client.guid
                or event.client.exactName) in self._BanList:
    ...
    def cmd_ragequit(
        ...
        if sclient:
            self.console.say('%s Just could not take it anymore! Good bye! (ragequit)'
                              % sclient.exactName)
            self.console.write('clientkick %s "GoodBye! Let The Great MW2 Gods Bless you with many more ^1Ragequits!"'
                                % sclient.cid)
            return True
    ...
    def cmd_rank(
        ...
        if not m:
            ...
            sclient = self._adminPlugin.findClientPrompt(m[0], client)
        if sclient:
            self.console.say('^7Setting ^1%s ^7Prestige to 11'
                             % sclient.exactName)
    ...
    def cmd_unlock(
        ...
        if not m:
            ...
            sclient = self._adminPlugin.findClientPrompt(m[0], client)
        if sclient:
            self.console.say('^7Unlocking ^1%s ^7From the server'
                             % sclient.exactName)
    ...
    def cmd_freeze(
        ...
        if not m:
            ...
            sclient = self._adminPlugin.findClientPrompt(m[0], client)
        if sclient:
            self.console.say('^7Freeze/Unfreezeing ^1%s'
                             % sclient.exactName)
    ...
    def cmd_space(
        ...
        if not m:
            ...
            sclient = self._adminPlugin.findClientPrompt(m[0], client)
        if sclient:
            self.console.say('^7Sending ^1%s ^7To space'
                             % sclient.exactName)
    ...
    def cmd_lock(
        ...
        if sclient:
            ...
            self.console.say('^7Locking ^1%s ^7Into the server ^3(cant leave) ^7For: ^1%s'
                              % (sclient.exactName, self._lockReason))
    ...
    def cmd_prestige(
        ...
        if sclient:
            ...
            if not m[1]:
                cmd.sayLoudOrPM(client,
                                '^1ERROR: ^7You must supply a Prestige!'
                                )
                return False
            else:
            ...
            time.sleep(0.2)
            self.console.say('^2Setting ^1%s ^2Prestige to: ^3%s!'
                             % (sclient.exactName, self._prestige))
            return True
    ...
    def cmd_bkick(
        ...
        if sclient:
            if sclient.maxLevel > client.maxLevel:
                ...
                self.console.write('clientkick %s "^7^1Kicked by:^5 %s ^1Reason of Kick: ^5 %s "'
                                    % (self._kickTarget,
                                   self._kickIniter, self._kickReason))
                self.console.say('^7Player: ^2 %s ^1Kicked By:^3 %s ^7For:^2 %s'
                                  % (self._kickTargetname,
                                 self._kickIniter, self._kickReason))
                return True
        return True
    ...
    def cmd_fps(
        ...
        if sclient:
            cmd.sayLoudOrPM(client,
                            'r_fullbright enabled/disabled! for %s'
                            % sclient.exactName)
...
...."""
        ...
        ip = m[0]
        cursor = \
            self.console.storage.query("SELECT id, name, time_edit FROM clients WHERE ip = '%s'"
                 % ip)
        if cursor.rowcount == 0:
            ...
            while not cursor.EOF:
                msg = ''
                r = cursor.getRow()
                msg += '^7[^2@%s^7] %s (^3%s^7)' % (r['id'], r['name'],
                        self.console.formatTime(r['time_edit']))
...
...."""
        ...
        if sclient:
            ...
            if cursor.rowcount == 0:
                ...
                cmd.sayLoudOrPM(client, 'Players that use %s: '
                                % sclient.ip)
                while not cursor.EOF:
                    msg = ''
                    r = cursor.getRow()
                    msg += '^7[^2@%s^7] %s (^3%s^7)' % (r['id'],
                            r['name'],
                            self.console.formatTime(r['time_edit']))
                    cmd.sayLoudOrPM(client, msg)
                    cursor.moveNext()
    ...
    def cmd_clientip(
        ...
        sclient = self._adminPlugin.findClientPrompt(m[0], client)
        if sclient:
            cmd.sayLoudOrPM(client, 'IP of %s: %s'
                            % (sclient.exactName, sclient.ip))
        return True
    ...
    def cmd_toadmins(
        ...
        for admin in adminList:
            if admin != client:
                admin.message('^3[from ^1%s^3]^7%s'
                              % (client.exactName, data))
    ...
    def cmd_pm(
        ...
        if sclient:
            ...
            msg = ' '.join(m[1:])
            sclient.message('^3[from ^1%s^3]^7%s' % (client.exactName,
                            msg))
        return True
